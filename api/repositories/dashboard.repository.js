...
export default class DashboardRepository {
    async obtenerEstadisticas() {
        ...
        const queryCursos = 'SELECT COUNT(*) as total FROM cursos WHERE id_curso_estado = 2';
        const queryCursosBorrador = 'SELECT COUNT(*) as total FROM cursos WHERE id_curso_estado = 1';
        ...
        const [resEstudiantes, resCursos, resCursosBorr, resAbiertos, resCerrados] = await Promise.all([
            pool.query(queryEstudiantes),
            pool.query(queryCursos),
            pool.query(queryCursosBorrador),
        ...
        return {
            ...
            totalBorradores: parseInt(resCursosBorr.rows[0].total),
